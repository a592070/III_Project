...
module.exports = {
  components: {
    ...
    'demo03': httpVueLoader(context + '/assets/a592070/front/travelSet/components/timeDemo03.vue'),
  },
  data() {
    return {
      dates: [],
  ...
  methods: {
        ...
        if(events.length > 0){
          for (let i = 0; i < events.length; i++) {
            if(events[i].time > item.time){
              ...
              // console.log("append after: " + this.events);
              break;
            }else if(events[i].time < item.time){
              events.push(item);
              // console.log("append before: " + this.events);
              break;
            }
          }
        }else{
    ...
    saveTravelSet () {
      ...
      // this.travelSetByDate[n-1] = this.events;
      // this.events = []
      //
      // if (n === this.dateNum) {
      //   this.e1 = 1
